export const WORLD_REGIONS = [
  ...
  },
  {
    ...
    unlockCondition: '7-day streak on any physical habit',
    unlockKey: 'streak_7',
  ...
  },
  {
    ...
    unlockCondition: '14-day streak on any habit',
    unlockKey: 'streak_14',
...
export function getUnlockProgress(region, playerData) {
  ...
  switch (unlockKey) {
    case 'streak_7':    return { current: Math.min(longestStreak, 7),   required: 7,  label: 'day streak' };
    case 'checkins_10': return { current: Math.min(totalCheckins, 10),  required: 10, label: 'check-ins' };
    case 'streak_14':   return { current: Math.min(longestStreak, 14),  required: 14, label: 'day streak' };
