...
export function getUnlockStatus(region, playerData) {
  ...
  // Journey system: regions reached by travel are unlocked permanently,
  // regardless of stat thresholds (which remain as an alternate path).
  if (Array.isArray(playerData.traveled) && playerData.traveled.includes(region.id)) {
    return true;
  }
