...
/**
 * Check if a profile represents a premium user.
 * Consolidates the various flags/columns that can indicate premium status.
 */
export function isPremiumUser(profile) {
  return !!(profile?.is_premium || profile?.subscription_status === 'active' || profile?.subscription_tier === 'pro');
}
