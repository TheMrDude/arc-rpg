...
const RATE_LIMITS = {
  ...
  'journal-create': {
    free: { limit: 10, window: 1 }, // 10 per minute
    premium: { limit: 10, window: 1 }, // 10 per minute
    burst: { limit: 10, window: 1 }, // Same as main limit
  },

  // Gold purchase (prevent spam)
  'purchase-gold': {
    free: { limit: 5, window: 1 }, // 5 per minute
    premium: { limit: 5, window: 1 }, // 5 per minute
    burst: { limit: 5, window: 1 }, // Same as main limit
  },

  // Seasonal event claims (prevent spam)
  'seasonal-claim': {
    free: { limit: 5, window: 1 }, // 5 per minute
    premium: { limit: 5, window: 1 }, // 5 per minute
    burst: { limit: 5, window: 1 }, // Same as main limit
...
async function isUserPremium(userId) {
  ...
  try {
    ...
    const isPremium = resolveIsPremium(data);
