...
export function getCompanion(profile, quests = []) {
  ...
  //
  // This matters more than it looks. Stage 0 is every brand-new user, which is
  // exactly who now names their companion during onboarding -- leaving the mood
  // line nameless meant the name appeared in two of three places for the only
  // audience that had just chosen it. Substituting the pronoun keeps all three
  // consistent. A line matching neither pattern renders unchanged.
  let mood;
  if (stage === 0) {
    mood = custom
      ? moodLine
