...
/**
 * Whether the naming prompt (the hatch) should open for this profile. One
 * condition does both jobs: a brand-new user hits it the instant their third
 * quest lands, and an account already past three quests hits it on its next
 * dashboard load — the catch-up path for anyone who crossed the threshold
 * before the naming flow shipped, or whose prompt never got answered.
 *
 * companion_name_prompted only turns true when the prompt is answered (named
 * or explicitly skipped), never merely shown, so closing the tab mid-hatch
 * re-prompts on the next visit instead of losing the moment.
 */
export function shouldPromptCompanionNaming(profile) {
  if (!profile) return false;
  if (profile.companion_name) return false;
  if (profile.companion_name_prompted) return false;
  return companionStage(profile.quests_completed || 0) >= 1;
}

/**
 * The next value for profiles.quests_completed after a completion.
 *
 * The hatch fires off this counter, so it must never drift low. A plain
 * `snapshot + 1` written from a profile read at the top of the request loses an
 * increment when two completions land together (both read N, both write N+1),
 * and a lost increment is an egg still sitting there after the third quest.
 * The completed quest rows are the authoritative record — the row is flipped
 * with an atomic completed=false guard before this runs — so the count of those
 * rows corrects any past drift, and `snapshot + 1` remains the floor for when
 * the row-count read itself fails (pass null) or lags.
 */
export function nextQuestsCompleted(profileCounter, completedRowCount) {
  const bumped = (profileCounter || 0) + 1;
  const counted = typeof completedRowCount === 'number' ? completedRowCount : 0;
  return Math.max(bumped, counted);
}
...
const MOODS = {
  ...
  activeToday: [
    ...
    // Cast cameos: the wider world notices too (characters from lib/cast.js).
    'spotted Gale Sprite doing loop-the-loops to celebrate your quest.',
    'heard Sunspin dance a whole extra lap on the sundial for you.',
  ],
  recent: [
    ...
    'traded stories about you with Satchel Raccoon. Good ones, obviously.',
    'is helping Puddle Wisp count raindrops until your next quest.',
  ],
  napping: [
    ...
    "is napping in Moss Warden's garden. Both will wake the moment you're back.",
    'curled up next to Cinderpaw by the warm coals, saving your spot.',
  ],
};
