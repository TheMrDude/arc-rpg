...
/**
 * WISDOM TREE EFFECTS
 * Enhance AI-generated content based on unlocked Wisdom skills
 */

/**
 * Get weekly summary enhancement level
 * @param {string} userId - User ID
 * @returns {Promise<number>} Enhancement level (0-3)
 */
export async function getWeeklySummaryEnhancement(userId) {
  const unlockedSkills = await getUserUnlockedSkills(userId);

  let level = 0;
  if (unlockedSkills.includes('wisdom_1')) level = 1; // Knowledge I: More detailed
  if (unlockedSkills.includes('wisdom_2')) level = 2; // Knowledge II: Enhanced narration
  if (unlockedSkills.includes('wisdom_5')) level = 3; // Omniscient: Perfect personalization

  return level;
}

/**
 * Get quest transformation enhancement level
 * @param {string} userId - User ID
 * @returns {Promise<number>} Enhancement level (0-3)
 */
export async function getQuestTransformEnhancement(userId) {
  const unlockedSkills = await getUserUnlockedSkills(userId);

  let level = 0;
  if (unlockedSkills.includes('wisdom_2')) level = 1; // Knowledge II: Enhanced
  if (unlockedSkills.includes('wisdom_5')) level = 2; // Omniscient: Perfect

  return level;
}

/**
 * Get story context memory depth
 * @param {string} userId - User ID
 * @returns {Promise<number>} Number of recent events to remember (5, 10, or 15)
 */
export async function getStoryContextDepth(userId) {
  const unlockedSkills = await getUserUnlockedSkills(userId);

  if (unlockedSkills.includes('wisdom_5')) return 15; // Omniscient
  if (unlockedSkills.includes('wisdom_3')) return 10; // Deep Insight
  return 5; // Default
}

/**
 * Check if user has Time Mastery for template optimization
 * @param {string} userId - User ID
 * @returns {Promise<boolean>}
 */
export async function hasTimeMastery(userId) {
  const unlockedSkills = await getUserUnlockedSkills(userId);
  return unlockedSkills.includes('wisdom_4');
}

/**
 * Get AI prompt enhancement based on Wisdom skills
 * @param {string} userId - User ID
 * @param {string} promptType - Type of prompt (quest, weekly, story)
 * @returns {Promise<string>} Additional instructions for AI
 */
export async function getAIPromptEnhancement(userId, promptType) {
  const unlockedSkills = await getUserUnlockedSkills(userId);
  let enhancements = [];

  if (promptType === 'weekly') {
    if (unlockedSkills.includes('wisdom_1')) {
      enhancements.push('Provide rich, detailed descriptions with vivid imagery and deeper character development.');
    }
    if (unlockedSkills.includes('wisdom_2')) {
      enhancements.push('Use advanced narrative techniques like foreshadowing, symbolism, and character arcs. Make it feel like a published fantasy novel.');
    }
    if (unlockedSkills.includes('wisdom_5')) {
      enhancements.push('Create a perfectly personalized epic that feels tailored specifically to this hero. Reference their past achievements and build a cohesive narrative arc.');
    }
  }

  if (promptType === 'quest') {
    if (unlockedSkills.includes('wisdom_2')) {
      enhancements.push('Transform this quest with epic, cinematic language. Make it feel legendary and important.');
    }
    if (unlockedSkills.includes('wisdom_5')) {
      enhancements.push('Create a perfectly personalized quest transformation that reflects this hero\'s journey and archetype in a unique way.');
    }
  }

  if (promptType === 'story') {
    if (unlockedSkills.includes('wisdom_3')) {
      enhancements.push('Incorporate more past events and maintain deeper continuity with previous story threads.');
    }
    if (unlockedSkills.includes('wisdom_5')) {
      enhancements.push('Create perfectly interconnected story threads that build toward an epic conclusion.');
    }
  }
