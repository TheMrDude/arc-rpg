...
export function getDashboardSections(profile) {
  ...
  return {
    ...
    // The companion is visible from the very first session, as an unhatched egg
    // with a live countdown on its card. That countdown is the first concrete
    // goal a brand-new account has, and it cannot do that job while hidden
    // behind a level gate. Naming happens when it hatches, at 3 quests.
    //
...
export function getNewUnlocks(sections, userId) {
  ...
  const unlockMessages = {
    ...
    // The companion is visible from the first load, and at that point it is an
    // unhatched egg. "A companion has joined your adventure" announced an
    // arrival that has not happened yet; the egg is a goal, not a gift.
    companion: { title: 'A Mysterious Egg', description: 'Complete 3 quests and it will hatch!', icon: '🥚' },
