...
// Interfaces removed for JavaScript compatibility
 ...
 */
export async function purchaseStreakFreeze(userId): Promise<PurchaseResult> {
  try {
 ...
 */
export async function getFreezeCount(userId): Promise<number> {
  try {
    const supabase = createClient();
 ...
 */
export async function isFreezeActive(userId): Promise<boolean> {
  try {
    const supabase = createClient();
