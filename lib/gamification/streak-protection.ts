 ...
 */
export async function purchaseStreakFreeze(userId: string): Promise<PurchaseResult> {
  try {
 ...
 */
export async function isFreezeActive(userId: string): Promise<boolean> {
  try {
    const supabase = createClient();
 ...
 */
export function calculateAffordableFreeezes(currentXP: number): number {
  return Math.min(
 ...
 */
export async function getStreakProtectionStatus(userId: string) {
  try {
    const supabase = createClient();
