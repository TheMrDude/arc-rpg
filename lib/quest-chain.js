...
export async function advanceWelcomeChain(userId, event) {
  try {
    const { data: progress } = await supabaseAdmin
      ...
      // Not enrolled is a normal state the next line already handles; .single()
      // would turn it into an error response instead.
      .maybeSingle();
    ...
    while (currentStep <= lastStepNumber) {
      ...
      const { data: updated } = await supabaseAdmin
        ...
        .select('current_step')
        // Zero rows is the expected outcome when another concurrent request
        // won the race, so it must not be reported as an error.
        .maybeSingle();
