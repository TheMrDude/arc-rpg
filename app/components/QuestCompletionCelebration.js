...
export default function QuestCompletionCelebration({
  ...
  // Priority-3 reward. This is the "+30 Gold" card that covered the Add Quest
  // button in the smoke log: on the shell now, so its ✕/Escape/backdrop close
  // paths are guaranteed and it can never sit on top of the dashboard with no
  // way past. `onClose` is routed through the queue's idempotent claimReward at
  // the call site, so the four close paths advance the reward flow exactly once.
  const visible = useOverlaySlot('quest-celebration', OVERLAY_PRIORITY.REWARD, show);

  useEffect(() => {
    if (!visible) return;
    // Trigger confetti when it reaches the screen (holds the slot), not merely
    // when `show` flips -- otherwise it celebrates behind whatever is up.
    const duration = 3000;
  ...
  const totalXP = (rewards?.xp || 0) + (rewards?.equipment_bonus_xp || 0);
