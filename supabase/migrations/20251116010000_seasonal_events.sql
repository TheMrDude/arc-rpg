...
-- FIXED: Changed from user_quests to quests table, and from status to completed field
...
-- FIXED: Trigger now uses 'quests' table and 'completed' field instead of 'user_quests' and 'status'
DROP TRIGGER IF EXISTS trigger_seasonal_quest_tracking ON quests;
...
DROP POLICY IF EXISTS "Users can update their challenge progress" ON user_seasonal_challenge_progress;
...
CREATE INDEX IF NOT EXISTS idx_seasonal_events_active ON seasonal_events(is_active) WHERE is_active = TRUE;
CREATE INDEX IF NOT EXISTS idx_seasonal_challenges_event ON seasonal_challenges(event_id);
CREATE INDEX IF NOT EXISTS idx_seasonal_rewards_event ON seasonal_rewards(event_id);
CREATE INDEX IF NOT EXISTS idx_user_seasonal_progress_user ON user_seasonal_progress(user_id);
CREATE INDEX IF NOT EXISTS idx_user_seasonal_progress_event ON user_seasonal_progress(event_id);
CREATE INDEX IF NOT EXISTS idx_user_seasonal_challenge_progress_user ON user_seasonal_challenge_progress(user_id);
CREATE INDEX IF NOT EXISTS idx_user_seasonal_challenge_progress_challenge ON user_seasonal_challenge_progress(challenge_id);

COMMENT ON TABLE seasonal_events IS 'Time-limited events with exclusive rewards and FOMO mechanics';
COMMENT ON TABLE seasonal_challenges IS 'Challenges users can complete during seasonal events';
COMMENT ON TABLE seasonal_rewards IS 'Rewards users can claim with event points';
COMMENT ON TABLE user_seasonal_progress IS 'Tracks user progress in seasonal events';
COMMENT ON TABLE user_seasonal_challenge_progress IS 'Tracks user progress on individual seasonal challenges';
